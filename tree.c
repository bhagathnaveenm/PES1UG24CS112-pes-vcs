...
// index_load is defined in index.c (linked into 'pes' but NOT into test_tree).
// Declaring it __attribute__((weak)) allows test_tree to link successfully;
// the symbol resolves normally when index.o is present in the full binary.
...
static int write_tree_recursive(IndexEntry *entries, int count, int prefix_len, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        if (!slash) {
            ...
            ObjectID sub_id;
            int new_prefix = prefix_len + (int)dir_name_len + 1; // +1 for the '/'
